...
public class SlobodniPaketBuilder implements PaketBuilder {
  ...
  public PaketBuilder postaviVisinu(String visina) {
    paket.setVisina(Double.parseDouble(visina.replace(',', '.')));
    return this;
  }

  @Override
  public PaketBuilder postaviDuzinu(String duzina) {
    paket.setDuzina(Double.parseDouble(duzina.replace(',', '.')));
    return this;
  }

  @Override
  public PaketBuilder postaviSirinu(String sirina) {
    paket.setSirina(Double.parseDouble(sirina.replace(',', '.')));
    return this;
  }
  ...
  public PaketBuilder naplata(String iznos) {
    if (paket.getUsluga_dostave().compareTo("P") == 0) {
      ...
      double konacna_cijena = osnovna_cijena
          + (paket.getDuzina() * paket.getSirina() * paket.getVisina() * paket.getVrsta().cijenaP())
          + (paket.getTezina() * paket.getVrsta().cijenaT());
      paket.setIznos_pouzeca(0);
      konacna_cijena = Math.round(konacna_cijena * 100);
      konacna_cijena = konacna_cijena / 100;
      paket.setIznos_dostave(konacna_cijena);
    }
    return this;
  }
